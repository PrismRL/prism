...
--- Computes the difference between this actor and another actor.
--- Diffs components by their className, calling each component's :diff method.
--- @param other Actor
--- @return { added?: table<string, Component>, removed?: table<string, boolean>, changed?: table<string, any> }|nil result
function Actor:diff(other)
   assert(other and other.className == Actor.className, "Actor:diff expected another Actor")

   local added, removed, changed = {}, {}, {}

   -- Removed and changed
   for _, comp in pairs(self.components) do
      local otherComp = other:get(getmetatable(comp))
      if not otherComp then
         removed[comp.className] = true
      else
         local d = comp:diff(otherComp)
         if d then
            changed[comp.className] = d
         end
      end
   end

   -- Added
   for _, otherComp in pairs(other.components) do
      if not self:get(getmetatable(otherComp)) then
         added[otherComp.className] = otherComp:serialize()
      end
   end

   -- Nil out empty subtables
   if next(added) == nil then added = nil end
   if next(removed) == nil then removed = nil end
   if next(changed) == nil then changed = nil end

   -- Return nil if absolutely no diffs
   if not (added or removed or changed) then
      return nil
   end
...
--- Applies a previously computed diff to this actor.
--- @param diff table|nil
function Actor:applyDiff(diff)
   if not diff then return end

   -- Removed components
   if diff.removed then
      for className in pairs(diff.removed) do
         local base = prism._OBJECTREGISTRY[className]
         if base then
            self:remove(base)
         end
      end
   ...
   -- Added components
   if diff.added then
      for _, ser in pairs(diff.added) do
         local comp = prism.Object.deserialize(ser)
         self:give(comp)
      end
   end
   ...
   if diff.changed then
      print "YEAH"
      for className, patch in pairs(diff.changed) do
         local comp = self:get(prism._OBJECTREGISTRY[className])
         if comp then
            comp:applyDiff(patch)
         end
      end
   end
end

function Actor:__serialize(ctx)
   local baseline = prism.resolveFactory(self.__factory)()
   local patch = baseline:diff(self)  -- may be nil if identical

   print("PATCH", patch)
   return {
      __factory = self.__factory,
      diff    = patch,  -- keep nil if no changes
   }
end

function Actor:onDeserialize()
   print(self.__factory, prism.json.encode(self.diff))
   local actor = prism.resolveFactory(self.__factory)()
   actor:applyDiff(self.diff)

   for k, v in pairs(actor) do
      self[k] = v
   end

   print(self:getPosition().x)
