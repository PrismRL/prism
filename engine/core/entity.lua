...
function Entity:applyDiff(diff)
...
function Entity:__serialize(ctx)
   ...
   return {
      ...
      __diff    = patch,  -- keep nil if no changes
   }
end

function Entity:__deserialize()
   local actor = prism.resolveFactory(self.__factory)()
   actor:applyDiff(self.__diff)
